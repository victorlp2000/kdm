...
import com.wel.kangmeida.preference.TZPreference;
import com.wel.kangmeida.tz.furuik.BitMapTools;
import com.wel.kangmeida.tz.furuik.BluetoothService;
import com.wel.kangmeida.tz.furuik.Confing;
import com.wel.kangmeida.tz.furuik.MyApplication;
import com.wel.kangmeida.tz.lefu.CRUtil;
import com.wel.kangmeida.tz.lefu.TZCFRecoder;
...
public class TZMainActivity extends SubActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AppPreferences.getInstance().init(getSharedPreferences("sb", Context.MODE_PRIVATE));
    }
    private void fanhui() {
    ...
    private void startMeasure() {
        ...
        if (TzUtil.TZ_DEVICE_LEFU.equals(tzDevice)) {//体脂秤lefu
            if ("启动".equals(action)) {
                // 还未绑定，开始绑定过程
        ...
        } else if (TzUtil.TZ_DEVICE_FURUIK.equals(tzDevice)) {//体脂秤furuik
            if (getPackageManager().hasSystemFeature(PackageManager.FEATURE_BLUETOOTH_LE)) {
                MyApplication.supportBle = true;
                BluetoothManager mBluetoothManager = (BluetoothManager) getSystemService(BLUETOOTH_SERVICE);
                MyApplication.mBluetoothAdapter = mBluetoothManager.getAdapter();
    ...
    protected void onStart() {
        ...
        if (bluetoothAdapter == null) {
            ...
            // 开启扫描
            String tzDevice = tzPreference.getTzDeviceModel();
            if (TzUtil.TZ_DEVICE_LEFU.equals(tzDevice)) {
                this.serviceIntent = new Intent(this, TimeService.class);
                this.startService(serviceIntent);
            } else if (TzUtil.TZ_DEVICE_FURUIK.equals(tzDevice)) {
                getApplicationContext().bindService(new Intent(TZMainActivity.this, BluetoothService.class), furuikConnection, Context.BIND_AUTO_CREATE);
            }
            // 注册广播
    ...
    public void enterTZSetting(View view) {
